...
class BundleBuilder {
	...
	async writeRawModule(module, resource) {
		...
		if ( /\.js$/.test(module) ) {
			const {compressedContent} = {...this.compressJS( fileContent, resource )};
				...
				if (mapping.source) {
					this._sourceMap.addMapping({
						...
						source: mapping.originalLine != null ?
							path.join(path.relative(path.dirname(this._sourceMap._file), path.dirname(resource.name)), ModuleName.getDebugName(path.basename(resource.name))) : null,
						name: mapping.name
					});
				}
			});
		...
		if ( this.options.usePredefineCalls ) {
			...
			for ( const module of sequence ) {
				if ( /\.js$/.test(module) ) {
					...
					let ast = rewriteDefine(this.targetBundleFormat, code, module, avoidLazyParsing);
					if ( ast ) {
						outW.startSegment(module);
						outW.ensureNewLine();
						...
						if (sourceMapContent) {
							astAsCode.map.applySourceMap(new sourceMap.SourceMapConsumer(sourceMapContent), ModuleName.getDebugName(path.basename(module)));
						}
...
function rewriteDefine(targetBundleFormat, code, moduleName, avoidLazyParsing) {
	...
	try {
		ast = esprima.parseScript(code.toString(), {
			comment: true,
			loc: true,
			source: ModuleName.getDebugName(path.basename(moduleName)),
			range: true,
			tokens: true
		});
	} catch (e) {
