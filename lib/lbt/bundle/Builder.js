		...
		if ( this.optimize ) {
			const result = terser.minify({
				...
				sourceMap: {
					...
					filename: `${path.basename(resource.name, ".js")}-dbg.js`,
					root: "."
				},
				...
				if (mapping.source) {
					this._sourceMap.addMapping({
						...
						source: mapping.originalLine != null ?
							path.join(path.relative(path.dirname(this._sourceMap._file), path.dirname(mapping.source)), `${path.basename(mapping.source, ".js")}-dbg.js`) : null,
						name: mapping.name
					});
				}
			});
