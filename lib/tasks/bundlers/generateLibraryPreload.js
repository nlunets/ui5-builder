...
function getBundleDefinition(namespace, isDesignTime) {
	...
	return {
		...
		defaultFileTypes: [".js", ".fragment.xml", ".view.xml", ".json"],
		sections: [
			{
				mode: "preload",
				name: `${namespace}/library-preload${isDesignTime ? ".designtime" : ""}`,
				filters: [
					`${namespace}/`,
					`!${namespace}/.library`,
					`!${namespace}/themes/`,
		...
		return p.then(() => {
			return workspace.byGlob("/resources/**/.library").then((libraryIndicatorResources) => {
				...
				return Promise.all(libraryIndicatorResources.map((libraryIndicatorResource) => {
					...
					const dtResources = resources.filter((resource) => {
						return path.dirname(resource.getPath()) === path.join(path.dirname(libraryIndicatorPath), "designtime");
					});
					if (libraryNamespaceMatch && libraryNamespaceMatch[1]) {
						const libraryNamespace = libraryNamespaceMatch[1];
						const bundles = [];
						bundles.push(moduleBundler({
							options: {
								bundleDefinition: getBundleDefinition(libraryNamespace)
							},
							resources
						}));
						if (dtResources.length > 0) {
							bundles.push(moduleBundler({
								options: {
									bundleDefinition: getBundleDefinition(`${libraryNamespace}/designtime`, true)
								},
								resources
							}));
						}
						return Promise.all(bundles).then((bundles) => {
							const outPromises = [];
							bundles.forEach(([bundle, sourceMapBundle]) => {
								if (bundle) {
