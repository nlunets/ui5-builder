...
module.exports = function({workspace, dependencies, options}) {
	...
	return combo.byGlob("/resources/**/*.{js,json,xml,html,properties,library}")
		...
		.then((processedResources) => {
			return Promise.all(processedResources.map(([componentPreload, componentPreloadMap]) => {
				const outPromises = [];
				if (componentPreload) {
					outPromises.push(workspace.write(componentPreload));
				}
				if (componentPreloadMap) {
					outPromises.push(workspace.write(componentPreloadMap));
				}
